...
class KidneyService:
    def checkKidney(self, data_dict):
        try:
            ...
            # Validate data
            if(age<0 and blood_pressure<0 and specific_gravity<0 and albumin<0 and sugar<0 and red_blood_cells<0 and pus_cell<0 and pus_cell_clumps<0 and bacteria<0 and blood_glucose_random<0 and blood_urea<0 and serum_creatinine<0 and sodium<0 and
              potassium<0 and haemoglobin<0 and packed_cell_volume<0 and white_blood_cell_count<0 and red_blood_cell_count<0 and hypertension<0 and diabetes_mellitus<0 and coronary_artery_disease<0 and appetite<0 and peda_edema<0 and aanemia<0):
                return -1
            # Preprocess data
            data = [[
                age, blood_pressure, specific_gravity, albumin, sugar, red_blood_cells, pus_cell, pus_cell_clumps, bacteria, blood_glucose_random, blood_urea, serum_creatinine, sodium,
                potassium, haemoglobin, packed_cell_volume, white_blood_cell_count, red_blood_cell_count, hypertension, diabetes_mellitus, coronary_artery_disease, appetite, peda_edema, aanemia ]]
